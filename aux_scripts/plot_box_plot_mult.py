    ...
    for exec in range(EXEC_NUMBER):
        ...
        with open(file_path,'r') as read_obj:
            ...
            for row in csv_reader:
                ...
                try:
                    values = row[0].split(';')
                    all_data.append([float(values[METRIC_INDEX]),load])
                except:
                    continue

data_df = pd.DataFrame(all_data, columns=['Data','Load'])
...
ax = sns.boxplot( x = 'Load', y = 'Data', data =data_df, linewidth=5, palette=colors)

# Gráfico de disperção
ax = sns.stripplot(x = 'Load', y = 'Data', data =data_df, hue="Load")  

# Adicionando Título ao gráfico
plt.title("Boxplot Cargas de Trabalho ", loc="center", fontsize=18)
plt.xlabel("Cargas de Trabalho")
plt.ylabel(f'{metrics[METRIC_INDEX]} ({metrics_unit[METRIC_INDEX]})')
