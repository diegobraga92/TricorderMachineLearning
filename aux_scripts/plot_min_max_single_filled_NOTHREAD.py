...
plt.title(f"MIN MAX - {metrics[METRIC_INDEX]} - SINGLE THREAD")
...
for idx,load in enumerate(LOAD_NAME):
    ...
    for sample_idx in range(SAMPLE_COUNT):
        ...
        if min([exec[sample_idx] for exec in execution_data]) > 0:
            minimum.append(min([exec[sample_idx] for exec in execution_data]))
        else:
            minimum.append(minimum[-1])

    plt.plot(maximum, color='r', label=f'{LOAD_NAME[idx]} - Max', alpha=0.90)
    plt.plot(minimum, color='y', label=f'{LOAD_NAME[idx]} - Min', alpha=0.90)
    plt.fill_between(range(100),maximum,minimum, color=colors[idx], label=LOAD_NAME[idx], alpha=0.20)
