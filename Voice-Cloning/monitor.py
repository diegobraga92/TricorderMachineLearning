...
class monitor:
    ...
    def __init__(self,sut,prefix,config,output_path,interval=0.2,sample_count=100, onefileconcat = False, show_plot=False):
        ...
        if sut[-18:] == "demo_cli_MEMORY.py":
            p = psutil.Popen([sys.executable, sut, "--low_mem","--no_sound", config[0], config[1]])
        else:
            p = psutil.Popen([sys.executable, sut, "--no_sound", config[0], config[1]])
