...
def start():
    ...
    if args.cmd == 'self':
        from .worker import self_play_MEMORY
        return self_play_MEMORY.start(config)
    elif args.cmd == 'opt':
        from .worker import optimize
        return optimize.start(config)
    elif args.cmd == 'eval':
        from .worker import evaluate_MEMORY
        return evaluate_MEMORY.start(config)
    elif args.cmd == 'sl':
        from .worker import sl_MEMORY
        return sl_MEMORY.start(config)
    elif args.cmd == 'uci':
